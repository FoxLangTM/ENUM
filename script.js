...
window.onload = () => {
    ...
    // Połączona obsługa potrójnego kliknięcia (Ghost Template / Zaznaczanie linii)
    ...
    // Numeracja linii - szybkie przewijanie
...
const toolbarActions = {
    console: () => { if (typeof toggleConsole === 'function') toggleConsole(); },
    paste: () => { paste(); },
    trash: () => { trash(); },
    undo: () => { undo(); },
    redo: () => { redo(); },
...
toolbar.addEventListener('touchend', stopContinuousAction);
...
toolbar.addEventListener('mouseup', stopContinuousAction);
...
function tinyHighlight(text) {
    ...
    function(match, comment, string, color, tag, keyword, punct) {
        ...
        if (color) return `<span class="color-preview" style="background: ${match};"></span><span class="token string">${match}</span>`;
        ...
        if (punct) {
            ...
            if (match === '#') {
                if (currentTab === 'js') return match;
                if (currentTab === 'css') return '<span class="token tag">' + match + '</span>';
            }
...
function startContinuousAction(actionFunc) { 
    stopContinuousAction(); 
    actionTimeout = setTimeout(() => { 
        actionInterval = setInterval(() => { actionFunc(); }, 60);
    }, 400);
}
...
document.querySelectorAll('.toolbar i').forEach(icon => {
    icon.addEventListener('click', () => {
        icon.classList.remove('tab-icon-click');
        void icon.offsetWidth;
        icon.classList.add('tab-icon-click');
    });
});
